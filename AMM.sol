...
contract AMM is AccessControl{
	...
	*/
	function tradeTokens( address sellToken, uint256 sellAmount ) public {
		...
		uint256 feeAdjustedAmount = (sellAmount * (10000 - feebps)) / 10000; // Adjust for fee

		//YOUR CODE HERE
		if (sellToken == tokenA) {
        qtyA = ERC20(tokenA).balanceOf(address(this));
        qtyB = ERC20(tokenB).balanceOf(address(this));
        swapAmt = qtyB - (invariant / (qtyA + feeAdjustedAmount));
        ...
        qtyA = ERC20(tokenA).balanceOf(address(this));
        qtyB = ERC20(tokenB).balanceOf(address(this));
        swapAmt = qtyA - (invariant / (qtyB + feeAdjustedAmount));
        require(swapAmt > 0, "Insufficient liquidity");
